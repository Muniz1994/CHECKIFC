...
def get_classification(element):
    """
    Retrieves the reference to a classification system of an element.
    ...
    classification = ifcopenshell.util.element.get_classification(element)

    """
    for assoc in element.HasAssociations:
        if assoc.is_a("IfcRelAssociatesClassification"):
            return assoc.RelatingClassification
...
filter_elements_grammar = lark.Lark(
    ...
    facet: instance | entity | attribute | type | material | query | classification | location | property | contained
    ...
    contained: "contained" comparison value
...
class FacetTransformer(lark.Transformer):
    ...
    def contained(self, args):
        ...
        def filter_function(element):
            ...
            result = False if containers else None
            for container in containers:
                if self.compare(container.is_a(), comparison, value):
                    result = True
            if result is not None:
                return result if comparison == "=" else not result
            return self.compare(None, comparison, value)
...
class Selector:
    @classmethod
    def parse(self, ifc_file, query, elements=None):
        self.file = ifc_file
        ...
        for child in group.children:
            if child.data == "query":
                ...
                if not lfunction:
                    ...
                    results = list(set(results).difference(new_results))
    ...
    def get_selector(self, selector):
        ...
        if not inverse_relationship:
            return results
        return self.parse_inverse_relationship(
    ...
    def get_class_selector(self, class_selector):
        if class_selector.children[0] == "COBie":
            ...
            if self.elements is None:
                ...
                if type(self.file) == ifcopenshell.file:
                    elements = self.file.by_type(class_selector.children[0])
                elif type(self.file) == ifcopenshell.entity_instance: 
                    elements =[self.file]                  
            else:
                elements = [
                    e for e in self.elements if e.is_a(class_selector.children[0])
                ]
        if (
    ...
    def filter_elements(self, elements, filter_rule):
        results = []
        filter_query = self.parse_filter_query(filter_rule.children[0].children[0])
        ...
        if len(filter_rule.children) > 1:
            ...
            if token_type == "ESCAPED_STRING":
                ...
                value = None
        for element in elements:
            element_value = self.get_element_value(element, filter_query["keys"], is_regex=filter_query["is_regex"])
            if element_value is None and value is not None and "not" not in comparison:
                continue
            if comparison and self.filter_element(
    ...
    @classmethod
    def parse_filter_query(self, filter_query):
        keys = filter_query
        is_regex = False
    ...
    @classmethod
    def get_element_value(self, element, keys, is_regex=False):
        value = element
        for key in keys:
            key = key.strip()
            if key == "type":
                ...
                value = get_classification(value)
                ...
                if key.isnumeric():
                    ...
                    results = []
                    for v in value:
                        subvalue = self.get_element_value(v, [key], is_regex=is_regex)
    ...
    def filter_element(self, element, element_value, comparison, value):
        if comparison.startswith("not"):
            return not self.filter_element(element, element_value, comparison[3:], value)
